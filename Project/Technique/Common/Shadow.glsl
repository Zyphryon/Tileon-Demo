...
float ShadowSwing(vec3 Point, vec3 Caster, float Pivot)
{
    return ZyWrapAngle(atan(Point.z - Caster.z, Point.x - Caster.x) - Pivot);
}
...
Arc ReadArc(vec3 Point, vec3 Origin, vec3 Caster)
{
    Arc Result;

    Result.Pivot  = atan(Origin.z - Caster.z, Origin.x - Caster.x);
...
vec4 PlaceAtlas(float Across, float Row)
{
    return vec4(Across * 2.0 - 1.0, Row * 2.0 - 1.0, 0.0, 1.0);
}
